    ...
    if (hand.confidence > 0.1) {
      for (let i = 0; i < hand.keypoints.length; i++) {
        ...
        if (handsDetect) {
          ...
          let middleTip = hand.keypoints[12]; // Tip of the middle//tip指尖 mcp指头最下面一节
          let middleMcp = hand.keypoints[9];
          dtip = dist(wrist.x, wrist.y, middleTip.x, middleTip.y);
          dmcp = dist(wrist.x, wrist.y, middleMcp.x, middleMcp.y);
          if (dtip > dmcp) {
            openHand = true;
            mist=false
          } else {
          ...
          if (openHand) {
            fill(0, 255, 0);
            textSize(32);
            text("Open Hand ✋", 20, 60);
            //text(d)
          } else {
            fill(255, 0, 0);
            textSize(32);
            text("Closed Fist ✊", 20, 60);
          }
        } else {
          //text("No hands", 20, 20);
        }
