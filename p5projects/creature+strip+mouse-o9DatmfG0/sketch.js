...
function drawCreature() {
  ...
  for (let r = 0; r <= 1; r += 0.04) {
    ...
    for (let theta = -360; theta <= 2700; theta += 2) {
      let phi = (180 / map(sin(frameCount * 2), -1, 1, 2, 10)) *
        Math.exp(-theta / (map(abs(sin(frameCount * 0.2 + PI)), 0, 1, 100, 10) * 180));
      let petalCut = 1 - (1 / 2) * pow((5 / 4) * pow(1 - ((3.6 * theta % 360) / 90), 2) - 1 / 4, 2);
      let hangDown = map(cos(frameCount * 2 +0.5), -1, 1, -3, 5) *
...
class Segment {
  ...
  update(index) {
    if (index === 0) {
  this.target = posVec.copy(); // Follow the flower creature!
